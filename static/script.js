...
function isArabicText(text) {
  ...
  // If a meaningful share of the text is Arabic script, treat it as Arabic.
...
function showWorkflow(data) {
  ...
  reasoning.classList.toggle("rtl", isArabicText(reasoning.textContent));
  chips.innerHTML = "";
...
function showResult(answer, threadId, isDraft = false) {
  ...
  pdfContent.classList.toggle("rtl", isArabicText(latestAnswerMarkdown));
...
function showApproval(data) {
  ...
  approvalRequest.classList.toggle("rtl", isArabicText(approvalRequest.textContent));
  section.classList.remove("hidden");
}
...
function withOriginCountry(message) {
  const originInput = document.getElementById("originCountry");
  const origin = (originInput && originInput.value.trim()) || "Egypt";

  // If the user already mentioned an origin ("from X"), leave their text as is.
  const mentionsOrigin = /\bfrom\b/i.test(message);

  if (mentionsOrigin) {
    return message;
  }

  return `${message} (traveling from ${origin})`;
}
...
async function sendMessage() {
  ...
  message = withOriginCountry(message);
...
function downloadPDF() {
  ...
  // Native browser printing renders real text (correct Arabic shaping and
  // RTL layout) instead of a rasterized screenshot, so it's used instead of
  // a canvas-based PDF library. The @media print rules isolate #pdfContent.
  window.print();
}
